            ...
            <div class="card">
                <h5 class="card-header">
                    <nav>
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">Công tơ điện</li>
                ...
                <div class="card-body">
                    <div class="col-12 d-sm-flex justify-content-between align-items-center">
                        <h5 class="card-title">Danh sách công tơ điện</h5>
                        @if (auth()->user()->hasPermissionTo('Thêm công tơ điện'))
                            <a href="{{ route('admin.cong_tos.dien.create') }}" class="btn btn-success rounded-pill">Thêm
                                công
                                tơ</a>
                        @endif
                    </div>
                    <form method="GET" action="{{ route('admin.cong_tos.dien.index') }}" class="row mb-3 align-items-end"
                        id="filter-form">
                        <div class="col-md-5">
                        ...
                        <div class="col-md-2 d-flex gap-2">
                            ...
                            <a href="{{ route('admin.cong_tos.dien.index') }}" class="btn btn-secondary w-100">Xoá lọc</a>
                        </div>
                    </form>
                        ...
                        <table class="table table-striped table-responsive">
                            <thead>
                                <tr>
                                    ...
                                    <th>Phòng</th>
                                    <th>Nhà trọ</th>
                                    @if (auth()->user()->hasAnyPermission(['Sửa công tơ điện', 'Xóa công tơ điện']))
                                        <th>Hành động</th>
                                    @endif
                            ...
                            <tbody>
                                @forelse ($congTos as $key => $ct)
                                    <tr>
                                        <td>{{ $key }}</td>
                                        <td>{{ ucfirst($ct->loai) }}</td>
                                        <td>{{ number_format($ct->chi_so_dau) }}</td>
                                        <td>{{ $ct->room->ten_phong ?? 'Chưa gán' }} -
                                            {{ $ct->room->ma_phong ?? 'Chưa gán' }}</td>
                                        <td>{{ $ct->nhaTro->ten_toa_nha ?? '' }}</td>
                                          @if (auth()->user()->hasAnyPermission(['Sửa công tơ điện', 'Xóa công tơ điện']))
                                        <td>
                                            @if (auth()->user()->hasPermissionTo('Sửa công tơ điện'))
                                                <a href="{{ route('admin.cong_tos.dien.edit', $ct) }}"
                                                    class="btn btn-sm btn-primary"><i class="icon-base bx bx-edit-alt"></i></a>
                                            @endif
                                            @if (auth()->user()->hasPermissionTo('Xóa công tơ điện'))
                                                <form action="{{ route('admin.cong_tos.dien.destroy', $ct) }}"
